...
**Phạm vi:** chỉ sửa tài liệu (`AR_TERRAIN_IOS_TOOLCHAIN_RUNBOOK.md`, mục này) và record ngoài Git; không phát hiện lỗi code mới, không sinh lại artifact (ZIP và SHA-256 `be66e489…c3e8` không đổi). Trạng thái các gate không đổi: `WINDOWS XCODE PROJECT GENERATION: PASS`; Mac Xcode build, ký Personal Team, cài/chạy iPhone, ARKit/LiDAR/GPS/RTK: **NOT RUN**. `8.1b` chưa hoàn tất; PR #80 giữ DRAFT, chưa merge.
