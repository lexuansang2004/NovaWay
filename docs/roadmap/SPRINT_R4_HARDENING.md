...
| R4-5 | ✅ Viết test cho `SocketIoRealtimeClient` và `GeolocatorLocationSource` (mobile) | P1 | Đây là code networking/platform **thật** đang chạy trong app — mọi test khác trong `apps/mobile/test/` chỉ exercise qua `FakeRealtimeClient`/`FakeLocationSource`, code thật chưa từng được test trực tiếp | **Hoàn tất 07/2026** — `GeolocatorLocationSource`: fake `GeolocatorPlatform.instance` (seam test chính thức của federated plugin, không phải fake toàn bộ `LocationSource`) nên logic thật của class (nhánh xin quyền, map `Position`→`LocationFix`, đổi m/s→km/h) chạy thật; 6 test (service tắt, đã có quyền sẵn, phải xin quyền, từ chối, deniedForever, map position stream đúng). `SocketIoRealtimeClient` không có seam tương đương (tự tạo `io.Socket` bên trong, không inject được) nên 4 test giới hạn ở phần quan sát được không cần server thật: `connect()` phát `connecting` đồng bộ trước I/O, `disconnect()`/`sendLocation()` gọi trước khi connect không throw (null-safe guard), và tuần tự state connect→disconnect không throw — vòng đời kết nối/nhận broadcast đầy đủ đã được phủ bởi E2E golden-path. Thêm `geolocator_platform_interface` vào `dev_dependencies` (trước là transitive-only, `flutter analyze` báo lỗi khi import trực tiếp). `apps/mobile/test/services/geolocator_location_source_test.dart`, `apps/mobile/test/services/socket_io_realtime_client_test.dart`. |
