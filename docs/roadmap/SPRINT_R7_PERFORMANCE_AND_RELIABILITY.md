...
| R7-3 | Backend chưa gọi `app.enableShutdownHooks()` — lifecycle hook (đóng kết nối DB, v.v.) không chạy khi process nhận tín hiệu dừng | P2 | Railway (production host đã chốt ở R3-6) gửi `SIGTERM` cho container cũ mỗi lần redeploy. Không có `enableShutdownHooks()`, NestJS **không** chạy `onModuleDestroy`/tương đương trước khi thoát — kết nối Postgres/WebSocket đang mở có thể bị cắt đột ngột thay vì đóng sạch | Grep xác nhận: không có `enableShutdownHooks`/`SIGTERM`/`SIGINT` ở `main.ts` hay bất kỳ đâu trong `apps/backend/src`. Đọc source `@nestjs/core` xác nhận cơ chế thật: `enableShutdownHooks()` (không truyền tham số) lắng nghe cả `SIGTERM` lẫn `SIGINT` qua `process.on(signal, cleanup)` chuẩn Node.js. **Giới hạn đã biết:** máy dev Windows không mô phỏng được `SIGTERM` thật như Linux (môi trường Railway) — `Stop-Process -Force` trên Windows tương đương `SIGKILL`, bỏ qua mọi signal handler. Verify khi làm mục này nên dùng `SIGINT` (Ctrl+C, hoạt động đúng trên cả Windows lẫn Linux, đi qua cùng code path `listenToShutdownSignals`) làm proxy hợp lệ cho cơ chế, thay vì cố mô phỏng đúng `SIGTERM` trên Windows |
| R7-4 | `AuthSession.isAuthenticated` (mobile) là dead code — khai báo nhưng không nơi nào gọi | P3 | Coverage `flutter test --coverage` (chạy lần đầu) chỉ ra `auth_session.dart` ở 67%; đọc ra đúng dòng thiếu là getter này. Grep xác nhận **0 call site** trong toàn bộ `apps/mobile/lib` lẫn `apps/mobile/test` | `apps/mobile/lib/session/auth_session.dart:10`. Xoá hoặc — nếu có ý định dùng sau này (vd. thay `AuthSession.token != null` rải rác bằng gọi hàm này) — wire vào ít nhất 1 chỗ thật, không để mồ côi |
...
- [ ] R7-1 hoàn tất — batch sync 500 event verify lại bằng chính phép đo ở mục 4, có cải thiện đo được so với baseline (3.277s).
- [ ] R7-2 hoàn tất — repro lại đúng kịch bản throw lỗi ở mục 4, xác nhận UI phục hồi hiện ra thay vì trang trắng.
...
**Ghi chú:** R7-3/R7-4 (P2/P3) không bắt buộc cho DoD tối thiểu, có thể kéo sang R8 nếu hết thời gian.
