...
|---|---|---|---|---|
| R2-1 | Nối `apps/web`'s `LiveMapPage` vào API thật: `POST /api/trips/start`, `/realtime` WebSocket thật (gửi vị trí trình duyệt thật hoặc mô phỏng qua API thay vì client-side thuần), `POST /api/trips/:id/end` | **P0** | Golden path hiện chỉ "thật" khi test qua E2E gọi thẳng REST/WS — qua UI thật vẫn là `useMockGpsSender` (`OPEN_ITEMS_AFTER_MVP.md` §9). Đây là gap sản phẩm lớn nhất còn lại từ MVP | Không phụ thuộc gì — có thể làm ngay |
| R2-2 | Build `POST /api/trips/sync` (batch sync) đúng contract đã đặc tả sẵn (`API_CONTRACT.md` §7: giới hạn 500 events/payload) + áp rate limit | P1 | Hoàn thành phần "N/A" của R1-4 — endpoint hiện chỉ có hợp đồng tài liệu, chưa từng implement (`OPEN_ITEMS_AFTER_MVP.md` §5) | Không phụ thuộc gì |
...
- [ ] R2-2 hoàn tất — `POST /api/trips/sync` hoạt động đúng contract, có rate limit, có test.
