...
| R2-3 | ✅ Migrate `apps/web`'s `TripMap.tsx`: Leaflet → MapLibre GL JS + wire Protomaps — verify trực quan xong qua browser thật — xem `docs/roadmap/OPEN_ITEMS_AFTER_MVP.md` §3 | P1 | Đóng quyết định đã chốt ở R1-7 (`docs/architecture/TDR-tile-provider-spike.md`) thành code thật; cần verify trực quan trong browser | **Bug thật đã tìm ra + fix:** `react-map-gl@8.1.1` chỉ tương thích thật với `maplibre-gl@^5.x` (peer dep ghi lỏng `>=4.0.0` gây hiểu nhầm v6 dùng được) — v6 khiến tile fetch network OK nhưng canvas không bao giờ render, không lỗi. Đã hạ về `maplibre-gl@5.24.0`, verify lại đúng qua Chrome thật của người dùng |
| R2-4 | ✅ Migrate `apps/mobile`'s `flutter_map` → `maplibre_gl` — verify trực quan xong qua `flutter run -d chrome` — xem `docs/roadmap/OPEN_ITEMS_AFTER_MVP.md` §7 | P1 | Đồng bộ tile source giữa web/mobile, đóng nốt phần "sẽ đổi" đã ghi ở OQ-006 (`OPEN_ITEMS_AFTER_MVP.md` §7) | **Bug thật đã tìm ra + fix:** `maplibre_gl_web` cần thẻ `<script>`/`<link>` nạp `maplibre-gl-js` thủ công trong `web/index.html`, không phải dependency Dart — thiếu bước này khiến map trống hoàn toàn, không lỗi. Đã thêm, verify lại đúng. **Đánh đổi đã chấp nhận:** chọn package `maplibre_gl` (ổn định) thay vì `maplibre` (mới hơn, hỗ trợ Windows) → mất khả năng verify qua Windows desktop như R1-5, chuyển sang `flutter run -d chrome`. **Còn nợ:** verify trên trình duyệt thật của người dùng (mới chỉ verify qua Browser pane), test Android/iOS thật (không có thiết bị) |
| R2-5 | ✅ Tự động hoá E2E-on-staging — workflow riêng `.github/workflows/e2e-staging.yml`, trigger push vào `develop` — xem `docs/roadmap/OPEN_ITEMS_AFTER_MVP.md` §2 | P1 | Hiện chỉ chạy thủ công nhắm staging thật; gate CI hiện tại (`e2e` job) chạy nhắm Postgres dựng trong runner, không phải staging thật (`OPEN_ITEMS_AFTER_MVP.md` §2) | **Trigger đã chốt** (hỏi người dùng): push vào `develop`, không phải cron/thủ công. **Fix cần thiết:** `playwright.config.ts`'s `webServer` phải bỏ hẳn khi nhắm site thật, nếu không CI sẽ spawn `pnpm dev` cục bộ dư thừa. **Còn nợ:** job không chờ deploy Railway/Vercel xong trước khi test (chỉ chờ `/health`) — chờ-deploy-xong đúng nghĩa để lại cho R2-7 |
