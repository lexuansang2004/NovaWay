...
| R-20 | AR Terrain Thesis Prototype (`8.x`, R&D độc lập, không phải MVP) chậm thiết bị: MacBook Air M3 đã có ngày 22/09 nhưng toolchain chưa PASS; iPhone 16 Pro chưa có ETA/chưa kiểm chứng. Lenovo + iPhone 11 Pro sẵn có. | Hiện hữu — `8.1b` có thể tiếp tục, lịch LiDAR vẫn AT RISK | Cao đối với R&D; không chặn roadmap MVP | Ưu tiên Xcode 16.4 + exact Unity/iOS Build Support + iPhone 11 smoke; `8.1b` vẫn bắt buộc trước `8.2`; không dùng Windows/iPhone 11 Pro thay LiDAR. Không tự lùi code freeze 15/11; Measurement Gate/Accuracy KPI tách biệt, không chỉnh số liệu. Xem baseline §4/§11/§13, `REVIEW_NOTES.md` §25. |
...
| R-21 | FPP/đồ hoạ AR hoặc việc chuyển camera khi lái làm người dùng nhìn màn hình lâu, hiểu nhầm độ chính xác overlay hoặc mất phương hướng | Trung bình nếu triển khai như game | Cao | Drive Mode mặc định TPP/map overview + âm thanh/rung; FPP chỉ bổ sung và ghi rõ nguồn map/AR; fallback về TPP khi tracking/camera lỗi; không có Second-Person runtime hay thao tác camera phức tạp khi xe chạy. Xem TDR-013/addendum Survey/Drive. |
