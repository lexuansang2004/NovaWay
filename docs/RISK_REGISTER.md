...
| R-20 | AR Terrain Thesis Prototype (nhóm step `8.x`, R&D độc lập cho báo cáo hội đồng, không phải MVP — baseline `docs/research/AR_TERRAIN_THESIS_BASELINE.md` **APPROVED** ở step `8.0` (2026-08-24)) trễ tiến độ, hoặc Accuracy KPI không đạt ngưỡng RMSE 3D ≤10cm, do phụ thuộc thiết bị AR thật (iPhone 15 Pro Max), toolchain Unity/Xcode chỉ chạy trên Mac, license Unity Student chưa xác nhận SheerID duyệt, và lịch thuê RTK theo đợt | Trung bình | Trung bình (không ảnh hưởng roadmap sản phẩm chính — track tách biệt) | Baseline + timeline + risk/fallback chi tiết ở `docs/research/AR_TERRAIN_THESIS_BASELINE.md` (§11 Timeline, §13 Risk & Fallback) — code freeze cứng `2026-11-15`, có điều kiện cảnh báo sớm (AT RISK) nếu chưa có mesh thật trước 07/09 hoặc `mesh_ar_local.ply` ổn định trước 04/10; Measurement Gate và Accuracy KPI tách biệt (§8) — nếu Accuracy KPI FAIL (RMSE >10cm), báo cáo trung thực thay vì chỉnh sửa số liệu, không coi là track thất bại nếu Measurement Gate vẫn PASS |
