...
- **Cập nhật hướng sản phẩm (09/2026):** tách Survey Mode (LiDAR acquisition/georeference) khỏi Drive Mode (GPS/heading/speed + terrain catalog quét sẵn). Track luận văn vẫn local/offline và độc lập khỏi `apps/*`; integration tương lai phải đi qua adapter/capability boundary, không import LiDAR implementation vào logic cảnh báo. TPP/map overview là view mặc định khi lái, FPP là bổ sung và Second-Person không thuộc runtime. Xem `docs/research/AR_TERRAIN_SURVEY_DRIVE_MODE_ADDENDUM.md` và TDR-013.
