...
describe('AuthService', () => {
  ...
  describe('login', () => {
    ...
    // R5-2: bcrypt was upgraded 5.1.1 -> 6.0.0 to drop the vulnerable
    // @mapbox/node-pre-gyp -> tar chain. Users registered before that upgrade
    // still have their original hash in the database, so this pins the thing
    // that would actually lock them out if it ever regressed. The hash below
    // is a real one generated by bcrypt 5.1.1 for 'password123' — do not
    // regenerate it, the whole point is that it predates the current version.
    it('accepts a password hash generated by the previous bcrypt major (5.1.1)', async () => {
      const hashFromBcrypt5 = '$2b$10$LGqenVbokq/LWgbpTVkzTOKD4hWH0MK6vS8glqBGppbvY.o9n8lUq';
      usersService.findByEmail.mockResolvedValue(buildUser({ passwordHash: hashFromBcrypt5 }));
      jwtService.sign.mockReturnValue('signed-jwt');

      await expect(authService.login('driver@example.com', 'password123')).resolves.toEqual({
        access_token: 'signed-jwt',
        user: { id: 'user-id', email: 'driver@example.com' },
      });
